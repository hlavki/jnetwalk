...
public class Board extends javax.swing.JPanel {
    ...
    private final static Map<Integer, Integer> contrDirs = initContrDirs();
    private final static int MINIMUM_CELLS = 20;
    ...
    public void newGame() {
        ...
        while(true) {
            ...
            for(int i = 0; i < getBoardSize() * getBoardSize(); i++) {
                ...
                if((d != Cell.FREE) && (d != Cell.NONE)) cellCount++;
            }
            if(cellCount >= MINIMUM_CELLS) break;
        }
    ...
    protected void addRandomDir(LinkedList<Cell> cellList) {
        ...
        if ((uCell != null) && uCell.getDirections() == Cell.FREE) freeCells.put(new Integer(Cell.U), uCell);
        if ((rCell != null) && rCell.getDirections() == Cell.FREE) freeCells.put(new Integer(Cell.R), rCell);
        if ((dCell != null) && dCell.getDirections() == Cell.FREE) freeCells.put(new Integer(Cell.D), dCell);
        if ((lCell != null) && lCell.getDirections() == Cell.FREE) freeCells.put(new Integer(Cell.L), lCell);
    ...
    private final static Map<Integer, Integer> initContrDirs() {
    ...
    public void rotate(int index, RotateDirection direction) {
        final int d = board[index].getDirections();
        if ((d == Cell.FREE) || (d == Cell.NONE) || isGameOver() || board[index].isLocked()) {
            blink(index);
        } else {
            //            board[index].rotate(direction.equals(RotateDirection.LEFT) ? -6 : 6);
            //            updateConnections();
            //            for(int i = 0; i < 14; i++) {
            //                board[index].rotate(direction.equals(RotateDirection.LEFT) ? -6 : 6);
            //                try {
            ...
            //                }
            //            }
            board[index].rotate(direction.equals(RotateDirection.RIGHT) ? -90 : 90);
    ...
    protected void endGame() {
        log.info(" Game with skill " + getSettings().getSkill() + " was successfully ended in " +
...
//        }
    ...
    public void setClickCount(int clickCount) {
        ...
        propertyChangeSupport.firePropertyChange("clickCount", new Integer(oldClickCount),
                new Integer(clickCount));
    }
